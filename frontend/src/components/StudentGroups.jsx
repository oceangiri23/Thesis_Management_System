...
const StudentGroups = () => {
  ...
  const fetchGroups = async () => {
    try {
      ...
      const endpoint =
        user.role === "supervisor" ? `/api/groups/supervised` : "/api/groups";
  ...
  const handleJoinRequest = async (groupId) => {
    try {
      const token = localStorage.getItem("token");
      await axios.post(
        `http://localhost:5000/api/groups/${groupId}/join-requests`,
        {
  ...
  const handleRequestResponse = async (requestId, status) => {
    try {
      const token = localStorage.getItem("token");
      await axios.put(
        `http://localhost:5000/api/groups/join-requests/${requestId}`,
        {
          status,
        },
        {
  ...
  const handleGroupApproval = async (
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.put(
        `http://localhost:5000/api/groups/${groupId}/approval`,
        {
          status,
  ...
  const userGroup =
    user.role === "student"
      ? groups.find((g) => g.Students?.some((s) => s.email === user.email))
      : null;
  ...
  return (
    <div className="student-groups-container">
      <div className="header">
        <h1>
          {user.role === "supervisor" ? "Supervised Groups" : "Student Groups"}
        </h1>
