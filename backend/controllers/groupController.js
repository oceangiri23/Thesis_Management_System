...
exports.createJoinRequest = async (req, res) => {
  try {
    ...
    if (student.semester !== group.semester) {
      return res
        .status(400)
        .json({ message: "You can only join groups from your semester" });
    }
    ...
    if (student.department !== group.department) {
      return res
        .status(400)
        .json({ message: "You can only join groups from your department" });
    }
  ...
  try {
    ...
    if (status === "accepted") {
      ...
      if (student.StudentGroupId) {
        ...
        return res
          .status(400)
          .json({ message: "Student is already in another group" });
      }
      ...
      if (student.semester !== group.semester) {
        ...
        return res
          .status(400)
          .json({ message: "Student must be from the same semester" });
      }
      ...
      if (student.department !== group.department) {
        ...
        return res
          .status(400)
          .json({ message: "Student must be from the same department" });
      }
...
exports.handleGroupApproval = async (req, res) => {
  try {
    ...
    //fetch supervisor email from req.user.id
    ...
    if (false) {
      console.log(group.SupervisorEmail, supervisorEmail);
